...
download.file("http://bit.ly/MS_trafficstops_bw_age",
              "C:/Users/luis0/Downloads/MS_trafficstops_bw_age.csv")

download.file("http://bit.ly/MS_acs_2015_bw",
              "C:/Users/luis0/Downloads/MS_acs2015_bw.csv")
...
```{r}
# complete the code
trf_stops <- read_csv("C:/Users/luis0/Downloads/MS_trafficstops_bw_age.csv")
...
selected_data <- dplyr::select(trf_stops, police_department, officer_id, driver_race)
...
# Youngest driver(s)
trf_stops %>% dplyr::slice_min(driver_age, n = 1)

# Oldest driver(s)
trf_stops %>% dplyr::slice_max(driver_age, n = 1)
...
```{r}
# Arrange the data by county_name and stop_date
arranged_data <- trf_stops %>%
  dplyr::arrange(county_name, stop_date)

# View the first record to get its ID
arranged_data %>% dplyr::slice(1) %>% dplyr::select(id)
```
...
# Filter for Black drivers and select officer_id and stop_date
black_stops <- trf_stops %>%
  dplyr::filter(driver_race == "Black") %>%
  dplyr::select(officer_id, stop_date)

# Count how many stops were made by officer D003
trf_stops %>%
...
age_summary <- trf_stops %>%
  dplyr::filter(!is.na(driver_race)) %>%
  dplyr::group_by(county_name, driver_race) %>%
  dplyr::summarize(mean_age = mean(driver_age, na.rm = TRUE),
                   min_age = min(driver_age, na.rm = TRUE))

# View summary for Amite County, white drivers
age_summary %>%
  dplyr::filter(county_name == "Amite", driver_race == "White")
...
Use the argument `sort = TRUE` in the parameters of `count()` to sort the data in descending order. 
 ...
 What is the officer_id with maximum stops? '''2940'''
 
 
 
**Solution**:
```{r}
# and sort in descending order by number of stops
officer_stops <- trf_stops %>%
  dplyr::count(officer_id, name = "n_stops", sort = TRUE)

# View the top officer (with maximum stops)
officer_stops %>% dplyr::slice(1)
...
''' The table summarizes the number of traffic stops (n_stops) by each officer for each type of violation, sorted from most to least frequent. '''
...
tot_pop <- read_csv("C:/Users/luis0/Downloads/MS_acs2015_bw.csv")
```
...
```{r}
stops_per_county <- trf_stops %>%
  dplyr::count(county_name, name = "n_stops")

combined_data <- stops_per_county %>%
  dplyr::left_join(tot_pop, by = c("county_name" = "county"))
...
# Create the violations table (long format)
violations <- trf_stops %>%
  dplyr::count(county_name, violation)

# Convert from long to wide format
violations_wide <- violations %>%
  tidyr::pivot_wider(
    names_from = violation,   # new column names will be the types of violations
    values_from = n           # fill cells with the counts from column 'n'
  )
