...
**Answer: The data is a health and nutrition survey of people living in the United States. It includes 76 different variables with each row represeting a detailed look at teach participant. **
...
**Answer: There were 7555 observations**
...
**Answer: A TP is a Yes and Yes, a TN is a No and No, a FP is a Truth No but Predicted Yes, and lastly a FN is a Truth Yes but Predicted No. **
...
Was the prediction correct? Discuss, again, the tree model based on the graph from **part 5) Yes it was**
...
# create training index (80% of rows)
train_index <- sample(1:nrow(people), size = 0.8 * nrow(people))

# training data
train <- people[train_index, ]

# testing data
test <- people[-train_index, ]
...
form <- as.formula(
  "Diabetes ~ Age + Gender + BMI + PhysActive"
)
...
# KNN model (you can try different k values)
set.seed(123)
knn_fit <- kknn(
  formula = form,        # Diabetes ~ Age + Gender + BMI + PhysActive
  train   = train,
  test    = test,
  k       = 15,          # choose k (e.g., 5, 10, 15…)
  scale   = TRUE
)

# predicted classes on the test set
knn_pred <- fitted(knn_fit)

# confusion matrix
table(Prediction = knn_pred, Truth = test$Diabetes)
...
# Confusion matrix
table(Prediction = nb_pred, Truth = test$Diabetes)

# Accuracy
nb_acc <- mean(nb_pred == test$Diabetes)
nb_acc
...
# Convert probabilities to Yes/No classes
log_pred <- ifelse(log_prob > 0.5, "Yes", "No")
log_pred <- factor(log_pred, levels = levels(test$Diabetes))

# Confusion matrix
table(Prediction = log_pred, Truth = test$Diabetes)

# Accuracy
log_acc <- mean(log_pred == test$Diabetes)
log_acc
...
**Answer: KNN scored 0.9152879 while Logistic Regression scored 0.9166115**
